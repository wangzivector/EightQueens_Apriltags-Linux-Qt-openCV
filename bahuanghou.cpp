...
unsigned int start_loca_set[18][7] = {
   { 8,15,22,29,36,43,57 },
   { 1,10,19,28,37,46,64 },
   { 57,49,41,33,25,17,9 },
...
unsigned int end_loca_set[18][7] = {
   { 24,13,30,47,35,60,1 },
   { 3,26,20,14,45,40,57 },
...
bahuanghou::bahuanghou(void)
{
     ...
     for(int countc=0;countc<8;countc++)
        chess_location[countc] = countc;
// chess_location[8] = { 18,29,37,41,42,49,52,61 };
// chess_location[8] = { 12,15,19,20,21,28,36,52 };
// chess_location[8] = { 2,5,8,12,30,32,36,47 };
// chess_location[8] = {1,8,13,21,31,40,46,58};

}

void bahuanghou::init()
{
    all_distance = 0;
    Min_all_distance = 10000;
    movestep=0;
    ...
    Max_Notmove = 0;
    Max_Notmove_times = 1;
    start_location=chess_location[0];
    end_location=65-chess_location[0];
}

void bahuanghou::swap(int *s1, int *s2) {
    int t = *s1;
    *s1 = *s2;
    *s2 = t;
}

void bahuanghou::reverse(int *s, int* e) {
    for (e--; s<e; s++, e--)swap(s, e);
}

int bahuanghou::next_permutation(int *start, int *end) {
...
unsigned int bahuanghou::abs(int value)
{
    return (value >= 0) ? value : -value;
}

void bahuanghou::check()
{
    int i, j, count_1;
	 int times;
    for ( times = 0; times < 92; times++)
    {
        i = 0;
        j = 0;
        count_1 = 0;
...
float bahuanghou::get_twopoint_dis(int start, int end)
{
    int x, y = 0;
...
int bahuanghou::check_move(int* start_loca, int* end_loca, int loca, int j)
{
	int i;
    for ( i = 0; i < Max_Notmove; i++)
    {
    ...
    while (b < Max_Notmove_times)
    {
        if (Notmove[a] == Max_Notmove)
        {
            ...
            do
            {
                do
                {
                    int  check_loca = 0, break_flag = 0;
									int j;
                    for ( j = 0; j < 8 - Max_Notmove; j++)
                    {
                    ...
                    if (break_flag == 0)
                    {
                        ...
                        for ( i = 0; i < 8 - Max_Notmove; i++)
                        {
                            dis1 += get_twopoint_dis(start_loca[i], end_loca[i]);
                        }
                        for ( i = 0; i < 7 - Max_Notmove; i++)
                        {
...
int bahuanghou::bahuanghou_figure(unsigned int *chessloca)
{
	init();
    for(int countc=0;countc<8;countc++)
       chess_location[countc] = chessloca[countc]+1;
     check();
        if (Max_Notmove < 2)		
        {
        ...
        one_solution();
           ...
           //执行完这个函数，起始的顺序在 min_start_loca[i]   其中数组个数为i =8 - Max_Notmove
						//	终点的顺序在min_end_loca[i] ;
        movestep=8 - Max_Notmove;
        for(int countq=0;countq<8;countq++)
        qDebug()<<"chess_location:"<<QString::number(chess_location[countq]);
        qDebug()<<"move step :"<<QString::number(movestep);
        for(int countq=0;countq<movestep;countq++)
        {
            qDebug()<<QString::number(countq+1)<<"step:"<<QString::number(min_start_loca[countq])<<"-->"<<QString::number(min_end_loca[countq]);
        }
        return 1;
}
